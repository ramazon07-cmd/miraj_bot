...
class ServiceListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        cat_title = self.kwargs.get('cat')
        lang = self.request.GET.get('lang', 'uz')
        cat = CategoryModel.objects.language(lang).filter(
            translations__title=cat_title).first()
        if cat:
            return ProductModel.objects.language(lang).filter(category=cat)
        else:
...
class Service2ListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if q:
            return ProductModel.objects.language(lang).filter(translations__title__icontains=q)
        else:
            return ProductModel.objects.none()
...
class KorzinCreateView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        qs = KorzinaModel.objects.filter(
            user_id=user_id, product=product, price=price)
        if not qs:
            KorzinaModel.objects.create(
                user_id=user_id, product=product, price=price, count=count)
...
class UserCountView(APIView):
    def get(self, request, *args, **kwargs):
        count = TelegramUserModel.objects.count()
